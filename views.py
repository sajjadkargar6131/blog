...
class PostListByTagView(generic.ListView):
    ...
    paginate_by = 6  # تنظیم تعداد پست‌ها در هر صفحه

    def get_queryset(self):
        tag_slug = self.kwargs['tag_slug']
        tag = get_object_or_404(Tag, slug=tag_slug)  # پیدا کردن تگ بر اساس slug
        return Post.objects.filter(tags=tag)  # فیلتر کردن پست‌ها بر اساس تگ

    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        tag_slug = self.kwargs['tag_slug']
        context['tag'] = get_object_or_404(Tag, slug=tag_slug)  # اضافه کردن تگ به context برای نمایش در قالب
        context['page_numbers'] = range(1, context['paginator'].num_pages + 1)  # برای صفحه‌بندی
        return context