...
from .models import Post, Like, BookmarkPost, PostView, Category, Comment
...
class PostDetailView(generic.DetailView, FormMixin):
    ...
    def get_context_data(self, **kwargs):
        ...
        comments_qs = self.object.comments.filter(publish=True).select_related('user').prefetch_related(replies_prefetch)
        ...
        short_link_obj, _ = ShortLink.objects.get_or_create(original_url=full_url)
        context['short_url'] = self.request.build_absolute_uri('/s/' + short_link_obj.short_code + '/')
...
def archive_month(request, year, month):
    ...
    posts = Post.objects.filter(shamsi_date__startswith=month_str, status='pub').select_related('author')
...
class PostListByTagView(generic.ListView):
    ...
    def get_queryset(self):
        ...
        return Post.objects.filter(tags=tag, status='pub').select_related('author')
